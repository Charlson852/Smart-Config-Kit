# v2rayN 使用教程（对齐 Clash Party v5.4.31）

> 目录简介：这里提供 v2rayN 多核心导入教程和 Xray 路由 JSON 兜底产物说明。
>
> 路径 C（Xray 核）产物：`v2rayN/v2rayN(xray).json` v5.4.31-v2n.1（详见 `v2rayN/CHANGELOG.md`；Xray 路由 JSON 不承载 DNS 解析，mihomo / sing-box 路径复用对应产物）。
...
| 规则数 | 382 providers | 39 remote rule_set + 710 route rules | 33 启用规则 |
...
这是 v2rayN 路径里**最接近 Clash Party 主线**的方案：33 业务组、22 区域 `url-test` 组、375+ rule-provider 全部原样生效，策略结构和 CMFA 完全一致。
