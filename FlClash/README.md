...
> 当前版本：**v5.4.34-flclash.1**（22 url-test 区域组 + 33 业务策略组；新增高德地图 / AMap 国内 API 前置直连；含借鉴 Proxy-override 批 A+B+C+D：国内 SDK/CDN 直连 + fake-ip-filter 补全 + direct-nameserver-follow-policy + 节点过滤 junk 关键词补充 + QUIC 精细化 + DoH-over-IP bootstrap）
