...
> 更新于 2026-05-30：default-nameserver 由"必须为 IP，不能为域名"修正为"必须为 IP，**可为加密 DNS**"。约束本质是 host 须为 IP（无需再解析、防 bootstrap 循环），但 scheme 可为 tls:// 或 https://——即 `tls://223.5.5.5`、`https://223.5.5.5/dns-query` 均合法，仅 `https://dns.example.com/...`（host 为域名）不合法。官方原文："必须为 IP, 可为加密 DNS"。
