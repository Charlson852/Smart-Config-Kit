...
> 版本：**v5.4.21-SR.1**（Build 2026-05-31，跟随 Clash Party v5.4.21 基线；含借鉴 Proxy-override 批 A #2 国内 SDK/CDN 直连 + 批 D #4 DoH-over-IP bootstrap）
...
- ❌ **导入配置时规则下载失败一半**：首次导入时 SR 要从 GitHub 拉约 290 个规则包。**必须先开代理再下载配置**。可以先用一个简单的代理配置连上，再切到本配置。
...
此时 Shadowrocket 会开始下载约 **290 个 rule-set**（blackmatrix7 Shadowrocket 专用 `.list` 格式），首次需要 **3–5 分钟**，期间请保持代理开启（否则 GitHub 访问不稳定会导致部分 rule-set 下载失败）。
...
| `proxy-dns-server` | `proxy-server-nameserver` | 境外 DoH 优先，国内 DoH 兜底 |
| `fallback-dns-server` | `fallback` | 国外 DoH（Cloudflare / Google） |

### 无法迁移的功能（已在配置内标注）
- Clash 的 `default-nameserver` **bootstrap**（SR 无独立字段，DoH 域名自举由客户端实现）
