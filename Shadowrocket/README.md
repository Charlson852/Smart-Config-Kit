...
> 版本：**v5.4.11-SR.1**（Build 2026-05-12，跟随 Clash Party v5.4.11 基线；RustDesk 域名走会议协作，DNS 调整为 IP-first）
...
| 字段 | 对应 Clash 字段 | 内容 |
|------|----------------|------|
| `dns-server` | `nameserver` + `direct-nameserver` | 国内 IP DNS 优先，DoH 兜底 |
| `proxy-dns-server` | `proxy-server-nameserver` | 代理服务器域名解析（IP bootstrap + DoH） |
